...
A separate **HubSpot Contact Deal** sub-workflow runs immediately after AI
qualification:

- Receives typed lead, match, AI, correlation, and trusted HubSpot
  configuration inputs directly from the parent.
...
HubSpot request nodes retry transient failures three times with a 30-second
wait. Ambiguous deal matches stop before either record is written. See
`docs/hubspot-crm-request-response.md` for the complete contract.
...
HubSpot contact/deal sub-workflow:
`workflows/ASSET001-hubspot-contact-deal.json`

The workflow exports use stable IDs. Import all four sub-workflows before the parent
...
Synthetic contact emails use `example.com`. Do not use the reserved `.test`
top-level domain for HubSpot contact writes because HubSpot rejects it as an
